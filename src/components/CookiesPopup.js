...
const CookiesPopup = ({ handleCookiesPopup, showCookiesPopup, showPop, setShowPop }) => {
  ...
  return (
    <>
      {showModal ? (
        <>
          <div className={`hidden overflow-x-hidden  fixed inset-0 z-50 outline-none focus:outline-none ${showCookiesPopup ? 'lg:hidden' : 'lg:block'}`}>
            <div className="w-[440px] absolute right-2 bottom-0 my-6 mx-auto max-w-3xl ">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="px-5 border-black border-opacity-10 rounded-t relative">
                  <Image src={cookies} alt="Cookies" className="w-[80px] absolute -top-8 -left-5" />
                  <h3 className="text-[26px] text-blue mt-[25px] mb-[17px] font-bold text-center">We use cookies!</h3>
                  <button className="bg-transparent border-0 text-black absolute top-[27px] right-[30px] focus:outline-none" onClick={() => setShowModal(false)}>
                ...
                <p className="text-[15px] font-jetBrain leading-[23px] font-normal text-center px-8 mb[30px]">
          ...
          {showPop ? <div className={`opacity-90 fixed inset-0 z-40 bg-white ${!showCookiesPopup ? 'lg:hidden' : 'lg:block'}`}></div> : null}
        </>
      ) : null}
    </>
  );
