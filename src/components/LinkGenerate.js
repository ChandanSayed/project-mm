...
const LinkGenerate = () => {
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-[5px] overflow-hidden lg:overflow-visible lg:bg-transparent lg:rounded-none">
            <div className="flex ">
              ...
              <div className="flex-1 flex">
                <p className={`flex-1 flex items-center font-jetBrain text-dark text-sm lg:text-[15px] border-l border-[#ddd] px-5 bg-white font-bold h-[50px] ${!secure && 'lg:rounded-[0_5px_5px_0]'}`}>{generatedLink}</p>
      ...
      <QRCodeComponent generatedLink={generatedLink} classes={`lg:hidden fixed pt-5 pb-8 bottom-6 left-0 right-0 w-full z-[60] qr-code bg-black bg-cover bg-no-repeat p-[10px] rounded-[10px] w-full overflow-hidden ${!showQr ? 'hidden' : 'block'}`} />
    </>
  );
};
