  ...
  return (
    <div className="autocomplete-input">
      <div className="relative">
        <ul className="autocomplete-options w-[220px] bg-[#000] text-white rounded-xl absolute bottom-full">
          {showList && <h3 className="py-[14px] pl-[22px] leading-[23px] border-b border-dark">Commands</h3>}
          {filteredOptions.map((option, index) => (
            <>
              <li key={index} className="py-[6px] pl-[22px] text-[#5e6372] hover:text-white leading-[23px] rounded-xl border-b border-dark bg-black cursor-pointer" onClick={() => handleSelectOption(option)}>
        ...
        <input value={inputValue} onChange={handleInputChange} onBlur={handleOnBlur} type="text" name="text" id="text" className="block w-full h-[50px] rounded-md border-0 py-1.5 pl-10 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400  sm:text-sm sm:leading-6" placeholder="" />
